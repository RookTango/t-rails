...
import { WatsonDeepChecklist } from '../components/watson/WatsonDeepChecklist';
...
const BOTTOM_TABS = [
  ...
  { key: 'watson-deep',  label: 'Watson Complex',       icon: FlaskConical },
];
...
function SectionHeader({ title }) {
  return (
    <div style={{ gridColumn: '1 / -1', background: '#e8ecf0', padding: '5px 10px', marginBottom: 4, marginTop: 8, borderLeft: '3px solid #1565c0', fontSize: 12, fontWeight: 700, color: 'var(--sn-text-primary)', letterSpacing: '0.02em' }}>
      {title}
    </div>
  );
}
...
function FormGrid({ children }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '160px 1fr 160px 1fr', gap: '2px 12px', alignItems: 'center', padding: '6px 0' }}>
      {children}
    </div>
  );
}
...
export default function ChangeDetailPage() {
  ...
  return (
    <div style={{ animation: 'fadeIn 0.2s ease', paddingBottom: 40 }}>
        ...
        {transitions.map(ts => {
          ...
          return (
            <button key={ts} onClick={() => allowed && handleTransition(ts)}
              ...
              style={{ ...btnBase, background: allowed ? (ts === 'CANCELLED' ? '#dc2626' : '#1565c0') : '#e5e7eb', color: allowed ? '#fff' : '#9ca3af', cursor: allowed ? 'pointer' : 'not-allowed' }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 8, paddingTop: 12, borderTop: '1px solid var(--sn-border-light)' }}>
              ...
              <button onClick={handleComment}
                style={{ padding: '6px 14px', background: 'var(--sn-blue)', border: 'none', borderRadius: 3, color: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5, fontSize: 13, fontFamily: 'inherit', alignSelf: 'flex-start' }}>
                <Send size={13} /> Post
              </button>
            </div>
          </div>
      ...
      {/* ── Bottom tabs: CIs / Tasks / Watson / Watson Complex ── */}
      <div style={{ marginTop: 8 }}>
