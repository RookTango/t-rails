...
from .models import ChangeRequest, ChangeTask, Attachment, ActivityLog, ChangeCI
...
class ChangeTaskSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model  = ChangeTask
        fields = '__all__'
        read_only_fields = ['task_number', 'created_at', 'updated_at', 'change', 'actual_start', 'actual_end']

    def get_ci_detail(self, obj):
        if obj.ci:
            return {'id': obj.ci.id, 'ci_id': obj.ci.ci_id, 'name': obj.ci.name, 'ci_type': obj.ci.ci_type}
        return None
...
class ChangeCISerializer(serializers.ModelSerializer):
    ...
    def get_ci_detail(self, obj):
        return {
            ...
            'environment': obj.ci.environment, 'ip_address': str(obj.ci.ip_address or ''),
